...
class HabitTestCase(APITestCase):
    ...
    def test_create_habit(self):
        ...
        self.assertEqual(
            ...
            {'id': 2, 'place': 'место', 'time_start': '08:00:00',
             'action': 'почистить зубы коту', 'is_pleasant': False,
             'period': '1', 'reward': 'скушать яблоко',
    ...
    def test_update_habit(self):
        ...
        response = self.client.put(f'/habit/{self.habit.id}/update/',
                                   data=data)
        self.assertEqual(
            response.status_code,
            status.HTTP_200_OK
        ...
        self.assertEqual(
            ...
            {'id': self.habit.id, 'place': 'test место',
             'time_start': '10:00:00', 'action': 'почистить зубы коту',
             'is_pleasant': False, 'period': '1',
    ...
    def test_partial_update_habit(self):
        ...
        response = self.client.patch(f'/habit/{self.habit.id}/update/',
                                     data=data)
        self.assertEqual(
            response.status_code,
            status.HTTP_200_OK
    ...
    def test_detail_habit(self):
        ...
        self.assertEqual(
            ...
            {'id': self.habit.id, 'place': 'test', 'time_start': '10:00:00',
             'action': 'testov', 'is_pleasant': False, 'period': '1',
             'reward': 'test testes', 'time_complete': 120,
             'is_published': False, 'owner': self.user.id,
             'related_habit': None}
        )
    ...
    def test_delete_habit(self):
        response = self.client.delete(f'/habit/{self.habit.id}/delete/')
        self.assertEqual(
            response.status_code,
